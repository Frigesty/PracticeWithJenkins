...
public class PracticeFormTests extends TestBase {
    ...
    void fullFillFormTest() {
        ...
        step("Вводим имя", () -> {
            registrationPage.setFirstName(gData.firstName);
        });

        step("Вводим фамилию", () -> {
            registrationPage.setLastName(gData.lastName);
        });

        step("Вводим email", () -> {
            registrationPage.setEmail(gData.userEmail);
        });

        step("Выбираем пол", () -> {
            registrationPage.chooseGender(gData.userGender);
        });

        step("Вводим номер телефона", () -> {
            registrationPage.setMobileNumber(gData.userNumber);
        });
        ...
        step("Выбираем предмет", () -> {
            registrationPage.setAndChooseSubject(gData.subject);
        });

        step("Выбираем хобби", () -> {
            registrationPage.chooseHobbies(gData.hobbies);
        });

        step("Загружаем картинку", () -> {
            registrationPage.uploadPicture(gData.pictures);
        });

        step("Вводим адрес", () -> {
            registrationPage.setAddress(gData.currentAddress);
        });
    ...
    void minimalFillFormTest() {
        ...
        step("Вводим пустое имя", () ->
                registrationPage.setFirstName("")
        );

        step("Вводим пустую фамилию", () ->
        ...
        step("Вводим некорректный номер телефона", () ->
                registrationPage.setMobileNumber("LL00011111")
        );
        ...
        step("Проверяем валидацию имени", () ->
                registrationPage.firstNameFieldInvalidationCheck()
        );

        step("Проверяем валидацию фамилии", () ->
                registrationPage.lasNameFieldInvalidationCheck()
        );

        step("Проверяем валидацию email", () ->
                registrationPage.userEmailFieldInvalidationCheck()
        );

        step("Проверяем валидацию гендера", () ->
                registrationPage.genderFieldInvalidationCheck()
        );

        step("Проверяем валидацию номера телефона", () ->
                registrationPage.mobileNumberFieldInvalidationCheck()
        );
    }
}